# Harmony Loader for Aotenjo

**中文版本可以在“[README.zh.md](README.zh.md)”中找到** (受限于我的中文水平，翻译质量可能不佳，欢迎提交改进建议！)
...
First, set up a mod in Aotenjo as you normally would using Lua as explained in the [official documentation](https://github.com/XOCatGames/Aotenjo-Src/wiki/1.-Getting-Started-%E5%BF%AB%E9%80%9F%E4%B8%8A%E6%89%8B). Once you have your mod structure ready, follow these steps to integrate Harmony Loader:
...
- Add references to the Harmony library, this library, Aotenjo's assemblies, and any other dependencies your mod requires.
  - Any assemblies that your mod depends on besides the three mentioned above must be placed in the `libs` folder of your mod directory.
...
- Create a class that extends `HarmonyLoaderAotenjo.HarmonyMod`.
  - Override the `ModName`, `ModAuthor`, and `ModVersion` properties to provide metadata about your mod.
  - The `Init()` method is called when your mod is loaded.
  - Patches are applied automatically when the mod is loaded, so you don't need to call `harmony.PatchAll()` manually.
- Implement your mod logic using Harmony patches and other C# features.
- Build your project to produce a DLL file.
- Place the compiled DLL into the `harmony` folder within your mod directory.
- Launch Aotenjo, and your Harmony patches should be applied automatically when your mod is loaded.

**It is recommended to use the official modding API (namespace `Aotenjo` in the game assemblies) whenever possible for better compatibility and stability as the API is designed to be stable across updates. Harmony patches should be used when the official API does not provide the necessary functionality.**
...
### Aotenjo Modding Documentation
[GitHub Wiki](https://github.com/XOCatGames/Aotenjo-Src/wiki/1.-Getting-Started-%E5%BF%AB%E9%80%9F%E4%B8%8A%E6%89%8B)
...
### Publicizing and Stripping Tools
